...
const {
  ...
  createReactComponentStories, createStylesComponent,
...
config.mergeAll({
  flags: options
    ...
    .filter((value) => value),
  root: ROOT_DIR,
  templates: `${ROOT_DIR}/.reactizzen/templates`,
  reactizzen: `${__dirname}/..`,
});
...
function createFiles(componentName, componentPath, cssFileExt) {
  return new Promise((resolve) => {
    ...
    if (cssFileExt && !config.hasFlag('reactnative')) {
      const cssModules = config.hasFlag('cssmodules');
      const cssStyles = config.hasFlag('cssstyles');

      let extension;
      if (cssModules) {
        extension = 'modules';
      } else {
        extension = cssStyles ? 'styles' : '';
      }

      files.push(`${name}.${extension}${cssFileExt}`);
    }
      ...
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if ((file.indexOf(`.stories.${ext}`) || file.indexOf(`.stories.${ext}`)) > -1) {
            ...
            promises.push(
              createStylesComponent(name).then((template) =>
                // eslint-disable-next-line implicit-arrow-linebreak
                fs.writeFileAsync(
                  filePath,
                  formatPrettier(template, prettierParser, prettierOptions),
