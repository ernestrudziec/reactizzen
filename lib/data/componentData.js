...
async function getComponentTemplate() {
  ...
  try {
    ...
    return readFileAsync(`${config.getValue('reactizzen')}/templates/${templateName}`);
  }
}
...
async function getTestTemplate() {
  try {
    ...
    return readFileAsync(`${config.getValue('reactizzen')}/templates/test.handlebars`);
  }
}
...
async function getStorybookTemplate() {
  try {
    ...
    return readFileAsync(`${config.getValue('reactizzen')}/templates/stories.handlebars`);
  }
}

/**
 * Get the styles component template
 *
 * @returns {Promise|Promise<string>}
 */
async function getStylesComponentTemplate() {
  try {
    return await readFileAsync(`${config.getValue('templates')}/styles.handlebars`);
  } catch (error) {
    return readFileAsync(`${config.getValue('reactizzen')}/templates/styles.handlebars`);
  }
}
 ...
 * @returns {String}
 */
async function createStylesComponent(componentName) {
  const name = stringHelper.componentNameWithoutSpecialCharacter(componentName);
  const file = getStylesComponentTemplate();
  const template = Handlebars.compile(await file);
