...
const reactizzenDir = '.reactizzen';
...
async function publish() {
  ...
  const promises = [];
  const projectTemplatePath = `${projectDir}/${reactizzenDir}/${templateDir}`;
  const libTemplatePath = `${libPath}/../${templateDir}`;

  try {
    await fs.createDirectorys(projectTemplatePath);
  } catch (error) {
    logger.error('Could not create directory .reactizzen', error);
    return;
  }
  ...
  try {
    ...
    logger.error('Could not read reactizzen\'s template directory', error);
    return;
  }
